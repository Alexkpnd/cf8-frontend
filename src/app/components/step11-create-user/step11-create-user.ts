...
import { FormArray, FormControl, FormGroup, AbstractControl, ReactiveFormsModule, Validators } from '@angular/forms';
...
export class Step11CreateUser {
  ...
  registratonStatus: { success:boolean, message:string} = {
    success: false,
    message: ""
  }
  ...
  onSubmit(){
    ...
    const userCreated = this.form.value as IUser

    this.userService.createUser(userCreated).subscribe({
      next: (response) => {
        this.form.reset()
        this.registratonStatus = {success:true, message: "User registered"}

      },
      error: (error) => {
        console.log('There was an error >>>', error);
        this.registratonStatus = {success: false, message: error.message }
      }
    })
  }
}
